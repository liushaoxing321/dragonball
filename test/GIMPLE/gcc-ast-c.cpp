...
static void pre_genericize(void *gcc_data, void *user_data) {
  ...
  // FIXME: only FUNCTION_DECL, but no VAR_DECL or other DECL in the Module level
  printf("DEBUG: %s, line %d: %d\n", __func__, __LINE__, TREE_CODE(decl));
  if (TREE_CODE(decl) == VAR_DECL) {
    tree id = DECL_NAME(decl);
    const char *VarName = id ? IDENTIFIER_POINTER(DECL_NAME(decl)) : "<unnamed>";
    printf("%s\n", VarName);
  } else if (TREE_CODE(decl) == FUNCTION_DECL) {
    tree id = DECL_NAME(decl);
    const char *FnName = id ? IDENTIFIER_POINTER(DECL_NAME(decl)) : "<unnamed>";
    printf("%s %s\n",
#if (GCC_MAJOR > 4)
    ...
    // FIXME: GCC v6.x work diff with v4.x
